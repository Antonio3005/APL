...
func CheckToken(tokenString string) (string, error) {
	...
	_, err = db.Exec(`CREATE TABLE IF NOT EXISTS token_revoked (
	...
	if err == nil {
		// Se il token è stato trovato nella tabella dei token revocati, restituisco un errore
	...
	// Verifico se il token è valido e non scaduto
	if claims, ok := token.Claims.(*Token); ok && token.Valid {
		...
		// Token valido, restituisco l'username
