...
func ShowPopular() ([]map[string]interface{}, error) {
	...
	// Creo una mappa per contare quante volte ogni coppia di città appare nelle ricerche
	counts := make(map[string]int)

	// Scorro i risultati della query e conto le occorrenze di ogni coppia città
	for rows.Next() {
		var cityFrom, cityTo string
		if err := rows.Scan(&cityFrom, &cityTo); err != nil {
			return nil, err
		}
		// Costruisco una stringa univoca per rappresentare la coppia città
		key := cityFrom + "|" + cityTo
		// Incremento il conteggio per questa coppia di città
		counts[key]++
	}
	...
	// Creare una map per memorizzare le ricerche più popolari
	var popularSearches []map[string]interface{}

	// Scorrere la mappa dei conteggi e aggiungere le ricerche più popolari alla mappa appena creataa
	...
	// Limito l'elenco alle prime 10 ricerche più popolari
...
// Funzione per estrarre le città
