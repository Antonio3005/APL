...
namespace APL
{
    public partial class Home : Form
    {
        ...
        private async void DisplayGraph()
        {
            string apiUrl = "http://127.0.0.1:5000/graph";
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    // Ottiengo i dati dell'immagine
                    byte[] imageData = await response.Content.ReadAsByteArrayAsync();

                    // Carico i dati dell'immagine in un MemoryStream
                    using (var stream = new System.IO.MemoryStream(imageData))
                    {
                        // Imposto l'immagine del PictureBox
                        pictureBox1.Image = Image.FromStream(stream);
                    }
                }
                else
                {
                    MessageBox.Show($"Errore durante il recupero del grafico: {response.StatusCode}");
                }
            }
            catch (HttpRequestException ex)
            {
                    ...
                    else
                    {
                        ...
                        if (pops.Count != 0)
                        {
                            foreach (var pop in pops)
                            {
                                ...
                                // Aggiungo etichette per visualizzare le informazioni del volo
